...
userMatches.forEach((match) => {
  ...
  if (modes[match.mode] !== undefined) {
    modes[match.mode] += 1;
  }
});

const bestCategory = Object.keys(categories).reduce((a, b) => (categories[a] > categories[b] ? a : b), "math");
...
if (typeof Chart !== "undefined") {
  ...
  new Chart(document.getElementById("modeChart"), {
    type: "doughnut",
    data: {
      labels: ["PVP", "Player vs PC", "Boss", "Tournament"],
      datasets: [{
        data: [modes.pvp, modes.pc, modes.boss, modes.tournament]
      }]
    },
    options: {
      responsive: true
